...
PG=repro-doltgresql-bug-function-execute-default-postgres
DG=repro-doltgresql-bug-function-execute-default-doltgresql
OUT=$(mktemp -d)
...
run() { # container, output file
  ...
  # repro.sql switches roles with \connect; every role's password is "password", so PGPASSWORD
  # covers each login. psql reads its standard input from /dev/null so that it does not print its
  # "psql (client version, server version)" line after each \connect, a line it prints only when
  # the two versions differ, as they do for DoltgreSQL.
  docker exec -t -e PGPASSWORD=password "$1" sh -c \
    'psql -X -P pager=off -h 127.0.0.1 -U postgres -d postgres --echo-all -f /tmp/repro.sql < /dev/null' |
    tr -d '\r' > "$2"
}
